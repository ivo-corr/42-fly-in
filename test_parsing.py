import sys
import unittest
from fly_in import parse_config

STRICT = "--strict" in sys.argv


class TestParsing(unittest.TestCase):
    def test_nb_drones_first_line(self) -> None:
        with self.assertRaises(Exception):
            with open("test_maps/01.txt") as file:
                parse_config(file.read())

    def test_negative_nb_drones(self) -> None:
        with self.assertRaises(Exception):
    ...
    def test_duplicate_end_hub(self) -> None:
        with self.assertRaises(Exception):
    ...
    def test_non_integer_coordinate(self) -> None:
        with self.assertRaises(Exception):
    ...
    def test_space_in_zone_name(self) -> None:
        with self.assertRaises(Exception):
            with open("test_maps/08.txt") as file:
                parse_config(file.read())

    def test_false_connection(self) -> None:
        with self.assertRaises(Exception):
            with open("test_maps/09.txt") as file:
                parse_config(file.read())

    def test_duplicate_connection_simple(self) -> None:
        with self.assertRaises(Exception):
            with open("test_maps/10.txt") as file:
                parse_config(file.read())

    def test_duplicate_connection_converse(self) -> None:
        with self.assertRaises(Exception):
    ...
    def test_zone_metadata_02(self) -> None:
        with self.assertRaises(Exception):
